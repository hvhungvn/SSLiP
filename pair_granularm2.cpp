...
/* ----------------------------------------------------------------------
   ...
   ****This file is modified by Viet Hung Ho (NTNU) with a modified Mindlin tangential contact ***
------------------------------------------------------------------------- */
...
PairGranularM2::PairGranularM2(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 1;
...
void PairGranularM2::compute(int eflag, int vflag)
{
  ...
  double pmax, pmax3; // calculation of Pmax at contact
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (!touchflag) {
		...
		///// This part is modified Mindlin (VHHo) Calculation of maximum contact pressure///////////
		
		pmax3 = 6*THREEQUARTERS*THREEQUARTERS*Fntot*normal_coeffs[itype][jtype][0]*normal_coeffs[itype][jtype][0]/(Reff*Reff*31.0062767);
		
		pmax= pow(fabs(pmax3),0.3333);
			
		////////////////////////////////////////////////////////
		
		////// Switch function with pmax with Pcrit=4GPa ans COF=0.5/////
		if (fabs(pmax) > 4000000 ) {
		  Fscrit = (0.5) * Fncrit;
		} 
		else {
...
void PairGranularM2::settings(int narg, char **arg)
{
  if (narg == 1) {
...
void PairGranularM2::coeff(int narg, char **arg)
{
...
void PairGranularM2::init_style()
{
  int i;
...
double PairGranularM2::init_one(int i, int j)
{
  double cutoff=0.0;
...
void PairGranularM2::write_restart(FILE *fp)
{
  int i,j;
  for (i = 1; i <= atom->ntypes; i++) {
...
void PairGranularM2::read_restart(FILE *fp)
{
  allocate();
  int i,j;
  int me = comm->me;
...
double PairGranularM2::single(int i, int j, int itype, int jtype,
                            ...
                            double /* factor_lj */, double &fforce)
{
  ...
  double pmax, pmax3;  // Calculation pmax
  ...
  if (normal_model[itype][jtype] == JKR) {
	...
	pmax3 = 6*THREEQUARTERS*THREEQUARTERS*Fntot*normal_coeffs[itype][jtype][0]*normal_coeffs[itype][jtype][0]/(Reff*Reff*31.0062767);
	pmax3 = 6*Fntot*E*E/(Reff*Reff*31.0062767);
	pmax= pow(fabs(pmax3),0.3333);
		
	////////////////////////////////////////////////////////

    ////// Switch function with pmax with Pcrit=4 GPa and COF=0.5/////
	if (fabs(pmax) > 4000000 ) {
	Fscrit = (0.5) * Fncrit;
	} 
	else {
...
int PairGranularM2::pack_forward_comm(int n, int *list, double *buf,
...
void PairGranularM2::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
...
double PairGranularM2::memory_usage()
{
...
double PairGranularM2::mix_stiffnessE(double Eii, double Ejj,
...
double PairGranularM2::mix_stiffnessG(double Eii, double Ejj,
                                    double poisii, double poisjj)
{
...
double PairGranularM2::mix_geom(double valii, double valjj)
{
...
double PairGranularM2::pulloff_distance(double radi, double radj,
                                      int itype, int jtype)
{
  double E, coh, a, Reff;
...
void PairGranularM2::transfer_history(double* source, double* target)
{
...
double PairGranularM2::atom2cut(int i)
{
  double cut;
...
double PairGranularM2::radii2cut(double r1, double r2)
{
  double cut = 0.0;
